...
export default async function buildGameDayResults(page, gameDay, currentDate) {
    ...
    for(let i = 0; i < gameDay.sameDayGames.length; i++){
        try {
                        ...
                        gameDay.sameDayGames[i].general.awayRecordFullPct = gameDay.sameDayGames[i].general.gameNumberAway > 1 ? `${(awayWins/(awayWins + awayLosses)).toFixed(3)}%` : '0.00%'
                        gameDay.sameDayGames[i].general.homeRecordFull = `${homeWins}-${homeLosses}`
                        gameDay.sameDayGames[i].general.homeRecordFullPct = gameDay.sameDayGames[i].general.gameNumberHome > 1 ? `${(homeWins/(homeWins + homeLosses)).toFixed(3)}%` : '0.00%'
                        ...
                        gameDay.sameDayGames[i].actual.awayFinalScore > gameDay.sameDayGames[i].actual.homeFinalScore ?  
                            (awayContextWins = parseInt(awayContextRecord.substring(0, awayContextRecord.indexOf('-'))) -1, 
                            awayContextLosses = parseInt(awayContextRecord.substring(awayContextRecord.indexOf('-') + 1)),
                            homeContextWins = parseInt(homeContextRecord.substring(0, homeContextRecord.indexOf('-'))),
                            homeContextLosses = parseInt(homeContextRecord.substring(homeContextRecord.indexOf('-') + 1)) - 1 
                            )
                            :
                            (awayContextWins = parseInt(awayContextRecord.substring(0, awayContextRecord.indexOf('-'))), 
                            awayContextLosses = parseInt(awayContextRecord.substring(awayContextRecord.indexOf('-') + 1)) - 1,
                            homeContextWins = parseInt(homeContextRecord.substring(0, homeContextRecord.indexOf('-'))) - 1,
                            homeContextLosses = parseInt(homeContextRecord.substring(homeContextRecord.indexOf('-') + 1))
                            ) 
                        ...
                        gameDay.sameDayGames[i].general.awayContextRecordFullPct = gameDay.sameDayGames[i].general.gameNumberAwayContext > 1 ? `${(awayContextWins/(awayContextWins + awayContextLosses)).toFixed(3)}%` : '0.00%'
                        gameDay.sameDayGames[i].general.homeContextRecordFull = `${homeContextWins}-${homeContextLosses}`
                        gameDay.sameDayGames[i].general.homeContextRecordFullPct = gameDay.sameDayGames[i].general.gameNumberHomeContext > 1 ? `${(homeContextWins/(homeContextWins + homeContextLosses)).toFixed(3)}%` : '0.00%'
                        ...
                        if(gameDay.sameDayGames[i].prediction.total > gameDay.sameDayGames[i].actual.total) {
                            ...
                            if(gameDay.sameDayGames[i].prediction.total == gameDay.sameDayGames[i].actual.total){
                                console.log(`Push for O/U ${gameDay.sameDayGames[i].general.away} @ ${gameDay.sameDayGames[i].general.home}`)
                                gameDay.sameDayGames[i].results.total = {
                                    'result': 'Push',
                                    'payout': 'Money Back'
                                }
